...
The figure~\ref{fig:CamerasParameters} first defines several generic geometrical
parameters, namely: $d_1$ the source to MPS-collimator distance, $d_2$ the
MPS-collimator to crystal distance, $D$ the MPS-collimator depth, $d_1^{'}$ the
source to KES-collimator center distance, $d_2^{'}$ the KES-collimator center to
crystal distance, $L_{MPS}=d_1+D+d_2$ or $L_{KES}=d_1^{'} + d_2^{'}$ the total
source-crystal distance, $H$ the camera height, $p$ the MPS collimator pitch, $f$
the fill factor (ratio between septa width and the pitch), $s$ the slit width
and $\alpha$ the KES-collimator angle. \answ{I would replace this paragraph by the following table} 
...
\answ{I tried a compromise with the ' notation to reflect the fact that these parameters play the same role in the cameras features.}
...
length of the radiation in the collimator material. \todo{Why not se for MPS ?} \answ{because the slabs of the MPS prototype have a negligible transparency.}
...
Per unit of bin length, the background yield of the MPS with ToF is therefore 4 times as low as the background seen with the KES. \answ{We do not apply background modeling in AMV anymore. The following sentence can be removed.\texttt{}} In the context of the fair comparison, for the KES camera the background with ToF can be obtained by multiplying the background with the same $\frac{4 ns}{10 ns} = 0.4$ fraction as with the MPS.
...
In the case of simulation for analytical model verifications, we use identical absorbers for both prototypes, namely the absorber of the KES prototype and we apply the same energy and TOF selection: the same energy selection ($E> 1$~MeV) and the same TOF selection (no TOF selection). Since AMV deals with gamma detection, no background modeling is applied in this AMV-specific setup. \todo{Question to Etienne: we discussed that for detection yield, we would not use the background. Then we should here explain clearly for which fig. of merit we use which background, and why. Agreed?}\answ{You are right, I removed the background modeling in this AMV study.}

Table~\ref{table:CameraParameters}\answ{Check the material used in \enquote{Prototypes comparison}. Did you use BGO for MPS?}{} gives an overview of the main cameras parameters used for AMV and prototypes comparison.
...
Figures~\ref{PGprofileFairComp}: PG profiles obtained with MPS and KES with the same absorber, the same energy E>1~MeV and no TOF selection.\answ{Figure to be updated. Current figure with a wrong energy selection.}
...
\begin{tabular}{llllllll}
	...
	\midrule
 	Res	& Real coll.		& 												& 17.8 mm 	 					&  											& 13.3 mm						&&\\
			& Perfect coll.	& 14.52 mm								& 18.3 mm 	 					& 13.5 mm								& 14.2 mm						&&\\
	Eff &								&  $6.66 \cdot 10^{-4}$ 	& $2.22\cdot10^{-5}$	&	$1.06 \cdot 10^{-3}$  & $3.96\cdot10^{-5}$& 1.59	&	\\
 	\midrule
\end{tabular}
\caption{MPS and KES detection efficiencies and spatial resolution from analytical models and MC. The predictions of the analytical model (AM column) are derived from the equations of table~\ref{table:AMformulas}, at the center of the camera in the case of KES ($x=0$). MC data have been obtained with energy selection E>1~MeV.}
\label{tab:AMV}
\end{table}
\answ{I am surprised by the KES spatial resolution obtained by MC. I would have expected a much large resolution due to the collimator transparency.}
\answ{Update the MC efficiency with the energy selection E>1~MeV. There is something weird: the spatial resolution obtained with MC increases when the collimator becomes perfect. I comment in the latex file the table with the spatial resolution obtained with MC with several TOF and energy selections.}
...
\textcolor{red}{We do not have this figure yet. The figure in the spot grouping paper shows the mean and standard deviations on the falloff position differences for the 3 spots considered. }\answ{The spot grouping paper results have been obtained with a voxellized geometry. In the present paper we use a homogeneous target (see M\&M section)}
...
\label{FRPCOMP}
\end{table}
\answ{What is the number of incident protons? For the same selections, we expect that KES has a better FRP than MPS thanks to its detection efficiency. It is far from being the case. That should come from a larger background level in KES (about twice the one of MPS in the background modeling). However this difference of FRP is quite surprising.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion}
